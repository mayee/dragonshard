...
public class SnowflakeIdGenerator {
    ...
    protected SnowflakeIdGenerator(long startTimestamp, long dataCenterId, long machineId) {
        ...
        if (startTimestamp > currentTimestamp) {
            throw ExceptionUtils.get(String.format("Start timestamp can't be greater than %d", currentTimestamp));
        }

        if (dataCenterId > MAX_DATA_CENTER_ID || dataCenterId < 0) {
            throw ExceptionUtils.get(String.format("Data center id can't be greater than %d or less than 0", MAX_DATA_CENTER_ID));
        }

        if (machineId > MAX_MACHINE_ID || machineId < 0) {
            throw ExceptionUtils.get(String.format("Machine id can't be greater than %d or less than 0", MAX_MACHINE_ID));
        }
